...
function App() {
  ...
  const updateInput = (value) => {
    ...
    if (symbols.slice(0, -2).includes(value)) { // if entered operator symbol, mark start of new number
  ...
  const updateResult = (new_input) => {
    ...
    if (new_input.length == 0) {
      setResult("");
    } else if (symbols.includes(new_input.slice(-1))){
      setResult(eval(new_input.slice(0, -1)).toString());
    } else {
      setResult(eval(new_input).toString());
    }
  };
    ...
    <div className="App">
      <div className="container">
        ...
        <div className="buttons">
          ...
          <button className="operators" onClick={() => updateInput("/")}>/</button>
